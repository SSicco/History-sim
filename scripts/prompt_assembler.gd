## Builds the 4-layer prompt structure for API calls.
## Layer 1: GM Guidelines (static, cached)
## Layer 2: Call-type template (changes by situation, cached)
## Layer 3: Juan II Profile (refreshed every 8 events, cached)
...
## Assembles the full 4-layer prompt for the current game state.
...
## Builds the 4-layer system content blocks with cache control markers.
func _build_system_blocks(call_type: String, player_input: String = "") -> Array:
	...
	# Layer 3 — Juan II Profile (cached, refreshes every 8 events)
	var profile_text := _build_profile_layer()
	if profile_text != "":
		blocks.append({
			"type": "text",
			"text": profile_text,
...
## Builds Layer 3: Juan II's profile.
func _build_profile_layer() -> String:
	...
	return "═══ JUAN II — PLAYER CHARACTER ═══\n" + profile_text


## Builds Layer 4: Scene-specific context from game data + sticky context.
func _build_scene_context(player_input: String = "") -> String:
	...
	if game_state.current_chapter > 0:
		parts.append("Chapter %d: %s" % [game_state.current_chapter, game_state.chapter_title])
	parts.append("")

	# Sticky context (characters, events, laws from ContextAgent)
	if sticky_context != null and sticky_context.has_data():
		parts.append(sticky_context.format_for_prompt())

	# Characters in scene that aren't already in sticky
	...
	if not game_state.scene_characters.is_empty() and not all_characters.is_empty():
		var sticky_char_ids: Array = []
		if sticky_context != null:
			sticky_char_ids = sticky_context.get_character_ids()

		var non_sticky_scene: Array = []
		for char_id in game_state.scene_characters:
			if char_id not in sticky_char_ids:
				var char_info = _find_character(all_characters, char_id)
				if char_info != null:
					non_sticky_scene.append(char_info)

		if not non_sticky_scene.is_empty():
			parts.append("═══ CHARACTERS PRESENT ═══")
			for char_info in non_sticky_scene:
				parts.append(_format_character_context(char_info))
				parts.append("")

	# Characters referenced in player input but not in scene or sticky
	if player_input != "" and not all_characters.is_empty():
		var referenced := _resolve_mentioned_characters(player_input, all_characters)
		var sticky_char_ids2: Array = []
		if sticky_context != null:
			sticky_char_ids2 = sticky_context.get_character_ids()

		var new_refs: Array = []
		for char_info in referenced:
			var cid: String = char_info.get("id", "")
			if cid not in game_state.scene_characters and cid not in sticky_char_ids2:
	...
	# Last 10 events (always included for continuity)
	var last_10_events := _get_last_n_events(10)
	if not last_10_events.is_empty():
		parts.append("═══ LAST 10 EVENTS ═══")
		for evt in last_10_events:
			var chars_str := ", ".join(PackedStringArray(evt.get("characters", [])))
...
## Returns the last N events from events.json.
func _get_last_n_events(n: int) -> Array:
	...
	var start_idx := maxi(0, all_events.size() - n)
	return all_events.slice(start_idx)
...
func _format_character_context(char_data: Dictionary) -> String:
	...
	if char_data.has("red_lines") and char_data["red_lines"] is Array and not char_data["red_lines"].is_empty():
		lines.append("Red Lines: %s" % ", ".join(PackedStringArray(char_data["red_lines"])))

	if char_data.has("event_refs") and not char_data["event_refs"].is_empty():
		var events_data = data_manager.load_json("events.json")
		if events_data != null and events_data.has("events"):
			lines.append("Recent Events:")
			for evt_id in char_data["event_refs"].slice(-5):
...
## (Legacy — kept for backward compatibility with PromptRetryManager)
...
func _build_character_index() -> String:
	...
	for c in all_chars:
		if not c is Dictionary:
			continue
		var char_id: String = c.get("id", "")
		var cname: String = c.get("name", char_id)
		var title: String = c.get("title", "")
		var location: String = c.get("location", "")
		lines.append("[%s] %s | %s | %s" % [char_id, cname, title, location])
